...
| - **SharePoint** ([Add OneDrive and SharePoint content](./build-declarative-agents.yml?tutorial-step=5)  **OR**  **[Optimize SharePoint content retrieval](optimize-sharepoint-content.md)) ? **| Mixed | SharePoint allows users to build declarative agents using data from specific SharePoint sites, document libraries, and files. | Business users who need agents that are grounded in specific SharePoint sites, document libraries, and files. |
