...
Code interpreter can generate and host downloadable files, which enables your agent to allow users to download files that the agent generates. When code interpreter runs and produces output files—such as images, charts, spreadsheets, or documents—Copilot presents a download link in the response. Users can select the link to download the file directly from the agent's response.
...
- **Generated visuals**: Produce a chart, QR code, or word cloud as an image file that the user can save.
- **Processed files**: Apply modifications to an uploaded image or document and return the result as a downloadable file.

To allow users to download a generated file, instruct your agent to save the output to a file and provide it for download. For example:

**Prompt:** *Create a spreadsheet with a multiplication table for numbers 1 through 10 and make it available as an Excel file.*

The agent generates the file, hosts it temporarily in the session, and presents a download link in the response that the user can select to download the file.

> [!NOTE]
> Files generated by code interpreter are available for download during the active session. They are not persisted after the session ends.
