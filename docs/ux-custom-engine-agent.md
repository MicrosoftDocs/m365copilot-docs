...
## Asynchronous patterns (preview)

Asynchronous patterns are non-blocking agent flow that a task doesn't depend on another, so users can keep chatting with an agents while a task is in process, even when the task takes a few minutes or days. Once the task is completed, the agent sends a notification to the user.
