...
To get started, open the Microsoft 365 Copilot app. In the left pane, select **Create agent**.
...
In addition to using the **Describe** tab, you can also directly configure the agent's behavior by using the **Configure** tab.
...
| **Icon**            | You can use AI to generate an icon, choose an icon from the icon library, or manually upload an image to represent your agent and give it a unique personality. To update the icon for your agent, select the pencil icon and choose **Generate**, **Browse**, or **Upload** to add a new icon.<br><br>If you upload an icon, a PNG file with a transparent background works best due to the extra padding that is applied by default around your icon.<ul><li>Supported file type: PNG</li><li>Color icon resolution limit: 192x192 pixels</li><li>File size limit: 1 MB</li></ul> <br>
For more details on icon specifications, see [Design icons for agent acquisition and management](/microsoft-365-copilot/extensibility/agent-icon-management). |
...
The agent in the right pane is a temporary instance of the agent that shows up as a side-by-side screen within the authoring experience. Use it to test the agent while you're creating or making updates to it. You can test your agent in the test pane to experience it in a similar way to end users. The test pane is enabled after the agent has a name, description, and instructions. It behaves according to those instructions like a fully fledged agent, including the ability to respond to complex queries within its realm of specified knowledge. The agent updates during each turn of the conversation as new information is added in the **Describe** or **Configure** tab.
...
In the **Capabilities** section of the **Configure** tab, choose to add the following capabilities to your agent:
