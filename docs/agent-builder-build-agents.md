...
| **Icon**            | You can use AI to generate an icon, choose an icon from the icon library, or manually upload an image to represent your agent and give it a unique personality. To update the icon for your agent, select the pencil icon and choose **Generate**, **Browse**, or **Upload** to add a new icon.<br><br>If you upload an icon, a PNG file with a transparent background works best due to the extra padding that is applied by default around your icon.<ul><li>Supported file type: PNG</li><li>Color icon resolution limit: 192x192 pixels</li><li>File size limit: 1 MB</li></ul> For more incormation about icons, see [Design icons for agent acquisition and management](/microsoft-365-copilot/extensibility/agent-icon-management). |
